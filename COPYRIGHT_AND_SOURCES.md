...
原书项目的许可包含非商业和禁止演绎限制。本教材仅参考其知识顺序、通用方法主题和公开链接，不发布整章翻译、原图、修改版 Notebook 或可替代原作的衍生文本。所有中文解释、示例和代码按课程需要独立组织。
...
- `figures/`中的教学结果图由本项目脚本使用合成数据生成，可通过`scripts/generate_textbook_figures.py`重现；替换为真实城市数据时需要同步更新图注、数据来源和适用范围。
